...
export default function AdminDashboardClient({ initialRole, initialEmail, error }: AdminDashboardClientProps) {
  ...
  return (
    <main className="min-h-screen pt-24 pb-16 px-6 max-w-7xl mx-auto flex flex-col md:flex-row gap-8">
      ...
      <aside className="w-full md:w-64 flex-shrink-0">
        <div className="bg-white/70 backdrop-blur-md border border-white p-6 rounded-[2rem] shadow-xl sticky top-28">
          <div className="mb-8">
            ...
            <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mt-1">{initialRole?.replace('_', ' ')}</p>
            {initialEmail && <p className="text-[10px] text-gray-400 mt-1 truncate">{initialEmail}</p>}
            {error && <p className="text-[10px] text-red-500 mt-2">Err: {error}</p>}
          </div>
      ...
      <div className="flex-grow">
        ...
        {activeTab === 'users' && initialRole === 'super_admin' && <UsersTab />}
